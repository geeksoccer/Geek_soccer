...
public class LiveScore_YesterdayView {
	...
	public View getView(Activity activity){
		...
		data.imageAdapterLiveScoreYesterdayByView = new ImageAdapter(
				mActivity.getApplicationContext());
		lstView.setAdapter(data.imageAdapterLiveScoreYesterdayByView);
		...
		if (data.Match_list_y_JSON.size() > 0) {
			...
			chk_ani = false;
			data.imageAdapterLiveScoreYesterdayByView.notifyDataSetChanged();
		} else {
				...
				if (ControllParameter.jObLiveYesterday != null) {
					...
					for (int i = 0; i < json_itArr.length(); i++) {
						...
						for (int j = 0; j < json_dtArr.length(); j++) {
							...
							} else if (SessionManager.chkFavContain(mActivity,
									id)) {
								if (!ContainFav) {
									ContainFav = true;
									data.Match_list_y_JSON
			...
			((Activity) mActivity).runOnUiThread(new Runnable() {
				public void run() {
					if (data.Match_list_y_JSON.size() > 0) {
						...
						((LinearLayout) layOutlist).addView(lstView);
						chk_ani = false;
						data.imageAdapterLiveScoreYesterdayByView
						...
						RefreshTag.setText(mActivity.getResources().getString(
